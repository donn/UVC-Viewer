...
namespace RB {
     ...
     * microcontrollers/single core) \tparam cacheline_size Size of the cache
     * line, to insert appropriate padding in between indexes and buffer \tparam
     * index_t Type of array indexing type. Serves also as placeholder for
    ...
    class Ringbuffer {
        public:
             ...
             * initialization code when object is instatiated in .bss section
             * \warning If object is instantiated on stack, heap or inside
             * noinit section then the contents have to be explicitly cleared
             * before use \param dummy Ignored
             */
            Ringbuffer(int dummy) { (void)(dummy); }

            /*!
             * \brief Clear buffer from producer side
             * \warning function may return without performing any action if
             * consumer tries to read data at the same time
             */
            void producerClear(void) {
                // head modification will lead to underflow if cleared during
                // consumer read doing this properly with CAS is not possible
            ...
            /*!
             * \brief Inserts data into internal buffer, without blocking
             * \param[in] data Pointer to memory location where element, to be
             * inserted into internal buffer, is located \return True if data
             * was inserted
             */
            ...
            /*!
             * \brief Inserts data returned by callback function, into internal
             * buffer, without blocking
             *
             * This is a special purpose function that can be used to avoid
             * redundant availability checks in case when acquiring data have a
             * side effects (like clearing status flags by reading a peripheral
             * data register)
             *
             * \param get_data_callback Pointer to callback function that
             * returns element to be inserted into buffer \return True if data
             * was inserted and callback called
             */
            ...
            bool remove() {
                ...
                if (tmp_tail == head.load(std::memory_order_relaxed))
                    ...
                    tail.store(
                        ++tmp_tail,
                        index_release_barrier
                    ); // release in case data was loaded/used before

                return true;
            }

            /*!
             * \brief Removes multiple elements without reading and storing it
             * elsewhere \param cnt Maximum number of elements to remove \return
             ...
             * \param[out] data Reference to memory location where removed
             * element will be stored \return True if data was fetched from the
             * internal buffer
             */
            bool remove(T &data) {
                return remove(&data
                ); // references are anyway implemented as pointers
            }

            /*!
             * \brief Reads one element from internal buffer without blocking
             * \param[out] data Pointer to memory location where removed element
             * will be stored \return True if data was fetched from the internal
             * buffer
             */
            bool remove(T *data) {
                index_t tmp_tail = tail.load(std::memory_order_relaxed);
             ...
             * \return Pointer to requested element, nullptr if index exceeds
             * storage count
             */
            T *at(size_t index) {
                index_t tmp_tail = tail.load(std::memory_order_relaxed);
             ...
             * \brief Gets the n'th element on consumed side
             *
             * Unchecked operation, assumes that software already knows if the
             * element can be used, if requested index is out of bounds then
             * reference will point to somewhere inside the buffer The isEmpty()
             * and readAvailable() will place appropriate memory barriers if
             * used as loop limiter It is safe to use and modify T contents only
             * on consumer side
             *
             * \param index Item offset starting on the consumed side
             * \return Reference to requested element, undefined if index
             * exceeds storage count
             */
            T &operator[](size_t index) {
                return data_buff
                    [(tail.load(std::memory_order_relaxed) + index)
                     & buffer_mask];
            }

            /*!
             * \brief Insert multiple elements into internal buffer without
             * blocking
             *
             * This function will insert as much data as possible from given
             * buffer.
             *
             ...
             * blocking
             *
             * This function will continue writing new entries until all data is
             * written or there is no more space. The callback function can be
             * used to indicate to consumer that it can start fetching data.
             *
             * \warning This function is not deterministic
             *
             * \param[in] buff Pointer to buffer with data to be inserted from
             * \param count Number of elements to write from the given buffer
             * \param count_to_callback Number of elements to write before
             * calling a callback function in first loop \param
             * execute_data_callback Pointer to callback function executed after
             * every loop iteration \return Number of elements written into
             * internal  buffer
             */
            size_t writeBuff(
                const T *buff,
            ...
            /*!
             * \brief Load multiple elements from internal buffer without
             * blocking
             *
             ...
             * blocking
             *
             * This function will continue reading new entries until all
             * requested data is read or there is nothing more to read. The
             * callback function can be used to indicate to producer that it can
             ...
             * \param[out] buff Pointer to buffer where data will be loaded into
             * \param count Number of elements to load into the given buffer
             * \param count_to_callback Number of elements to load before
             * calling a callback function in first iteration \param
             * execute_data_callback Pointer to callback function executed after
             * every loop iteration \return Number of elements that were read
        ...
        private:
            ...
            static_assert(
                buffer_mask <= ((std::numeric_limits< index_t >::max)() >> 1),
    ...
    template <
        ...
        typename index_t >
    size_t
    Ringbuffer< T, buffer_size, fake_tso, cacheline_size, index_t >::writeBuff(
        const T *buff,
        size_t count
    ) {
        index_t available = 0;
    ...
    Ringbuffer< T, buffer_size, fake_tso, cacheline_size, index_t >::writeBuff(
        ...
        while (written < count) {
            available =
                buffer_size - (tmp_head - tail.load(index_acquire_barrier));
    ...
    template <
        ...
        typename index_t >
    size_t
    Ringbuffer< T, buffer_size, fake_tso, cacheline_size, index_t >::readBuff(
        T *buff,
        size_t count
    ) {
        index_t available = 0;
